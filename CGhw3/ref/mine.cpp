...
void getYcoordinate(int* Ycoordinate, int pi, int dx, int dy, int index, int length);
// 得到y坐标
void getYcoordinate(int* Ycoordinate, int pi, int dx, int dy, int index, int length) {
	if (index == length - 1) {
		// 计算完成，此处为终点坐标
	...
	int parm2 = 2 * dy - 2 * dx;
	if (pi <= 0) {
		Ycoordinate[index + 1] = Ycoordinate[index];
		pi += parm1;
	} else {
		Ycoordinate[index + 1] = Ycoordinate[index] + 1;
		pi += parm2;
	}
	index += 1;
	getYcoordinate(Ycoordinate, pi, dx, dy, index, length);
}
